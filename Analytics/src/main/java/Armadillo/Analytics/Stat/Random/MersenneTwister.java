...
*/
public class MersenneTwister implements IRng {
	
	private int m_intMti;
	private int[] m_intMt = new int[N]; /* set initial seeds: N = 624 words */
	private final Object m_seedLock = new Object();
...
private void nextBlock() {
	...
	synchronized(m_seedLock){
		...
		for (kk=0;kk<N-M;kk++) {
			y = (m_intMt[kk]&UPPER_MASK)|(m_intMt[kk+1]&LOWER_MASK);
			m_intMt[kk] = m_intMt[kk+M] ^ (y >>> 1) ^ ((y & 0x1) == 0 ? mag0 : mag1);
		}
		for (;kk<N-1;kk++) {
			y = (m_intMt[kk]&UPPER_MASK)|(m_intMt[kk+1]&LOWER_MASK);
			m_intMt[kk] = m_intMt[kk+(M-N)] ^ (y >>> 1) ^ ((y & 0x1) == 0 ? mag0 : mag1);
		}
		y = (m_intMt[N-1]&UPPER_MASK)|(m_intMt[0]&LOWER_MASK);
		m_intMt[N-1] = m_intMt[M-1] ^ (y >>> 1) ^ ((y & 0x1) == 0 ? mag0 : mag1);
...
public int nextInt() {
	...
	synchronized(m_seedLock){
		/* Each single bit including the sign bit will be random */
	  	if (m_intMti == N) nextBlock(); // generate N ints at one time
...
private void setSeed(int seed) {
	synchronized(m_seedLock){
		m_intMt[0] = seed & 0xffffffff;
		for (int i = 1; i < N; i++) {
			m_intMt[i] = (1812433253 * (m_intMt[i-1] ^ (m_intMt[i-1] >> 30)) + i); 
			/* See Knuth TAOCP Vol2. 3rd Ed. P.106 for multiplier. */
			/* In the previous versions, MSBs of the seed affect   */
			/* only MSBs of the array mt[].                        */
			/* 2002/01/09 modified by Makoto Matsumoto             */
			m_intMt[i] &= 0xffffffff;
			/* for >32 bit machines */
	 	}
 ...
 */
public static MersenneTwister makeDefault() {
