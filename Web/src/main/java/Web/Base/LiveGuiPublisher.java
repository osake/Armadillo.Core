...
import Armadillo.Communication.Impl.AStringCallback;
...
public class LiveGuiPublisher 
{
	...
	private static void loadCallback() 
	{
		try
		{
			SimpleUiSocket.addCallback(new AStringCallback() 
			{
				@Override
				...
				public void runTask(ObjectWrapper item) 
				{
					while(true)
					{
						try
						{
							...
							publishTableRow("test1", 
									"test1", 
									m_rng.NextInt(1, 10) + "_test", 
									m_rng.NextInt(1, 10) + "_testKey", 
									fooList.get(0));
							Thread.sleep(500);
						}
						catch(Exception ex)
			...
			String str3)
	{
		try
		{
			...
			AUiChartItem uiChartItem = getChartUiItem(
					str1, 
					str2, 
					str3);
			...
			Object obj) 
	{
		try
		{
			if(StringHelper.IsNullOrEmpty(str1) ||
				StringHelper.IsNullOrEmpty(str2) ||
				StringHelper.IsNullOrEmpty(str3) ||
				StringHelper.IsNullOrEmpty(strObjectKey) ||
				obj == null)
			{
				return false;
			}
			
			AUiTableItem uiTableItem = getUiItem(
					str1, 
					str2, 
					str3, 
					obj);
	...
	private static void publishChart(String[] toks) 
	{
		try
		{
			...
			{
				Console.writeLine("Could not parse to double token " + tsToks[1]);
			}
		}
		catch(Exception ex)
		{
...
//	public static void PublishLineChart(
//			String str1, 
//			String str2,
//			String str3,
...
//						str2, 
//						str3);
//				uiChartItem.populateLineChartRow(strSeriesName, strX, dblY);
//				//uiChartItem.SetXMaxVal(dblY);
//		}
//		catch(Exception ex)
//		{
//			Logger.log(ex);
...
//	
			...
			String[] vals) 
	{
		try
		{
			...
			m_tablePublishQueue.add(strKey, 
									publishJob);
			strKey = null;
		}
		catch(Exception ex)
			...
			String[] vals) 
	{
		...
		{
			//
			// TODO, implement efficient queue here!!
			//
			...
			AUiTableItem uiTableItem = getTableUiItem(
					str1, 
					str2, 
					str3, 
					cols);
	...
	public static boolean PublishBubbleChartRow(
			String str1, 
			String str2,
			String str3, 
			String strSeries,
			...
			String strLabel) 
	{
		try
		{
			...
			uiChartItem.populateBubbleChartRow(
					strSeries, 
					intX, 
					intY, 
					intRadious, 
	...
	private static AUiTableItem getUiItem(
			String str1, 
			String str2,
			String str3, 
			Object obj) 
	{
		try
		{
			String strTreeKey = UiHelper.getNodeKey(
					str1, 
					str2,
					str3);
			...
			{
				synchronized(LockHelper.GetLockObject(strTreeKey + "_publisherLock"))
				{
					if(!m_ownInstance.m_guiItems.containsKey(strTreeKey))
					{
						String[] treeLabels = new String[]{
															str1,
															str2,
															str3
														};
						Reflector reflector = ReflectionCache.getReflector(obj.getClass());
			...
			AUiTableItem uiTableItem = (AUiTableItem) m_ownInstance.m_guiItems.get(strTreeKey);
			return uiTableItem;
		}
		catch(Exception ex)
		{
	...
	public static AUiTableItem getUiItem(
			String str1, 
			String str2,
			String str3) 
	{
	...
	private static AUiTableItem getTableUiItem(
			String str1, 
			String str2,
			String str3, 
			String[] colNames) 
	...
	private static AUiChartItem getChartUiItem(
			String str1, 
			String str2,
			String str3) 
	{
		try
		{
			...
			AUiChartItem uiChartItem = (AUiChartItem) m_ownInstance.m_guiItems.get(strTreeKey);
			return uiChartItem;
		}
		catch(Exception ex)
		{
	...
	private static AUiTableItem generateCustomUiChartItem(
			final String strTreeKey, 
			final String[] treeLabels) 
	{
		try
		{
	...
	public static void initialize() 
	{
		...
		{		
			...
			m_tablePublishQueue = new EfficientProducerConsumerQueue<PublishJob>(1, 10000)
					{
						@Override
